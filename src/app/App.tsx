...
/**
 * StudyTales App - Main application component
 *
 * This is the heart of the game. It manages:
 * - Navigation between different game sections
 * - Game state (coins, cards, XP, etc.)
 * - Toast notifications for user feedback
 * - Smooth page transitions with animations
 *
 * The app uses a tab-based navigation system to keep everything modular
...
export default function App() {
   ...
   * This gives the player nice feedback when they earn rewards or level up
   */
  ...
  /**
   * Handle coin earnings from various game activities
   * Logs the reason so we can track what players are doing
   */
  ...
  /**
   * When a card is awakened to a higher tier
   * This is a big moment for players, so we celebrate it!
   */
  const handleAwakenCard = useCallback(
    (characterId: number) => {
  ...
  /**
   * Record a battle victory and reward the player
   * This is one of the core gameplay loops
   */
  ...
  /**
   * Render the appropriate content based on which tab is active
   * Each section has its own component for clarity and maintainability
   */
  const renderContent = () => {
    switch (activeTab) {
      case 'home':
  ...
  return (
    ...
    >
      {/* Scrollable content area with smooth page transitions */}
      ...
      <nav
        ...
        style={{
          ...
          boxShadow:
            '0 -4px 24px rgba(124,58,237,0.07), 0 -1px 0 rgba(124,58,237,0.06)',
        }}
      >
        <div className="flex">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {/* Notification dot for gacha pulls */}
                  {item.id === 'gacha' && (
                    <span
      ...
      {/* Toast notification area - Shows at top center */}
      <div
          ...
          {toasts.map((toast) => (
            <motion.div
              ...
              style={{
                ...
                boxShadow:
                  '0 8px 32px rgba(124,58,237,0.18), 0 2px 8px rgba(0,0,0,0.08)',
