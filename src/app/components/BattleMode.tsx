...
// Logic beats Emotion, Emotion beats Strength, Strength beats Logic
...
// per-character state during battle
interface BattleCharState {
  char: Character;
  hp: number;
  maxHp: number;
  effectiveAtk: number;
...
// demo team so new players can jump in without grinding gacha first
...
export function BattleMode({ ownedCards, onEarnCoins, onRecordWin, equippedFrame }: Props) {
  ...
  // chars the player actually owns
  ...
  const startBattle = (team = playerTeam) => {
    ...
    // awakened cards get a big hp/atk boost
    const initPlayer = (ids: number[]): BattleCharState[] => ids.map(id => {
      const char      = CHARACTERS.find(c => c.id === id)!;
      const ownedCard = ownedCards.find(oc => oc.characterId === id);
      const awake     = ownedCard?.awakened === true;
      const hp        = awake ? Math.floor(char.hp  * 1.25) : char.hp;
      const atk       = awake ? Math.floor(char.atk * 1.5)  : char.atk;
      return { char, hp, maxHp: hp, effectiveAtk: atk, isAwakened: awake };
    });

    // enemy team is always at base stats
  ...
  const doAction = useCallback((action: "attack" | "skill" | "defend") => {
    ...
    if (action === "attack") {
      ...
      // use the awakened skill name if available
      const skillName = attacker.isAwakened && attacker.char.awakenedSkill
        ? attacker.char.awakenedSkill
        : attacker.char.skill;
    ...
    setTimeout(() => {
      setEnemyStates(current => {
        ...
        setPlayerStates(prev => {
          const next = [...prev];
          if (next[pTargetIdx]) {
            next[pTargetIdx] = { ...next[pTargetIdx], hp: Math.max(0, next[pTargetIdx].hp - eDmg) };
          }
    ...
    return (
      <div className="max-w-2xl mx-auto">
        ...
        <div style={{ padding: "10px 12px", display: "flex", flexDirection: "column", gap: 10 }}>
                    ...
                    <div key={id} style={{ flex: 1, position: "relative" }}>
                      ...
                      {isAwakened && (
                        <div className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full flex items-center justify-center"
                          style={{ background: "#f59e0b", boxShadow: "0 0 8px rgba(251,191,36,0.7)", zIndex: 20 }}>
          ...
          {/* always show demo team button — useful even if you have your own cards */}
          ...
          <button onClick={() => startBattle()} disabled={playerTeam.length === 0}
            style={{
              ...
              fontFamily: "'VT323', monospace", fontSize: "1.3rem", letterSpacing: "0.1em",
              background: playerTeam.length === 0 ? "#fca5a5" : "#dc2626",
  ...
  if (state === "result") {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center h-full">
        ...
        <p style={{ fontFamily: "'VT323', monospace", fontSize: "2.2rem", letterSpacing: "0.08em", color: winner === "player" ? "#059669" : "#dc2626" }}>
          {winner === "player" ? "VICTORY!" : "DEFEATED"}
        </p>

        {winner === "player" && (
          <div style={{ fontFamily: "'VT323', monospace", fontSize: "1.4rem", color: "#d97706", marginTop: 10 }}>
            🪙 +150 COINS
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 20, width: "100%", maxWidth: 280 }}>
          ...
          <button onClick={() => startBattle()}
            style={{
              ...
              fontFamily: "'VT323', monospace", fontSize: "1rem", letterSpacing: "0.06em",
              background: "#dc2626", color: "white",
              border: "2px solid #b91c1c", borderRadius: 4, cursor: "pointer",
  ...
  return (
    <div className="h-full flex flex-col p-3 max-w-2xl mx-auto gap-2">
      ...
      <div style={{
        ...
        <div style={{ background: "#c4a868", padding: "3px 10px", display: "flex", alignItems: "center", gap: 5 }}>
          <span style={{ fontFamily: "'VT323', monospace", fontSize: "0.75rem", color: "white", letterSpacing: "0.08em" }}>
            📜 BATTLE LOG
          </span>
        </div>
            ...
            return (
              <div key={i} style={{ flex: 1, opacity: ps.hp === 0 ? 0.3 : 1, filter: ps.hp === 0 ? "grayscale(1)" : "none" }}>
                <div style={{
                  ...
                  border: `2px solid ${isActive ? "#059669" : "#a7f3d0"}`,
                  boxShadow: isActive ? "0 0 8px rgba(16,185,129,0.4)" : "none",
                }}>
                  {/* awakened art shows here since isAwakened is in ps */}
                  <CardImage character={ps.char} size="xs" showName isAwakened={ps.isAwakened} />
                </div>
      ...
      <div className={`grid grid-cols-3 gap-2 flex-shrink-0 ${(turn !== "player" || isAnimating) ? "opacity-40 pointer-events-none" : ""}`}>
        <button onClick={() => doAction("attack")}
          style={{
            ...
            background: "#dc2626", color: "white",
            border: "3px solid #b91c1c", borderRadius: 6, boxShadow: "3px 3px 0 #7f1d1d",
        ...
        <button onClick={() => doAction("skill")} disabled={skillCooldown > 0}
          style={{
            padding: "12px 8px", fontFamily: "'VT323', monospace",
            cursor: skillCooldown > 0 ? "not-allowed" : "pointer",
            background: skillCooldown > 0 ? "#4c1270" : "#7c3aed", color: "white",
            border: "3px solid #5b21b6", borderRadius: 6,
            boxShadow: skillCooldown > 0 ? "none" : "3px 3px 0 #4c1270",
            opacity: skillCooldown > 0 ? 0.5 : 1,
            display: "flex", flexDirection: "column", alignItems: "center", gap: 3,
        ...
        <button onClick={() => doAction("defend")}
          style={{
            padding: "12px 8px", fontFamily: "'VT323', monospace", cursor: "pointer",
            background: isDefending ? "#ddeef6" : "#5b9aba",
            color:      isDefending ? "#1a3d52" : "white",
      ...
      {turn === "player" && !isAnimating && (isDefending || skillCooldown > 0) && (
        <p className="text-center text-xs flex-shrink-0 vt" style={{ color: "#5a7d8a", fontSize: "0.8rem" }}>
          {isDefending
            ? "🛡️ Defending — next hit blocked 50%"
            : `⚡ Skill ready in ${skillCooldown} turn${skillCooldown > 1 ? "s" : ""}`
          }
        </p>
      )}
    </div>
